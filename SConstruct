...
options(env,
  ...
  ('cxxflags_extra','',['-Wno-undefined-var-template','-Wno-instantiation-after-specialization', '-Wno-unknown-warning-option', '-w']),
  ('linkflags_extra','',[]),
...
if env['openmp']:
  if not gnuc:
    ...
    print('Warning: clang doesn\'t know how to do OpenMP, so many things will be slower', file=sys.stderr)
...
def external(env,name,lazy=1,**kwargs):
  ...
  def fail():
    ...
    if kwargs.get('required'):
      print('FATAL: %s is required.  See config.log for error details.'%name, file=sys.stderr)
      Exit(1)
...
def external_helper(env,name,default=0,dir=0,flags=(),cxxflags='',linkflags='',cpppath=(),libpath=(),rpath=0,libs=(),publiclibs=(),
  ...
  if configure is not None:
    ...
    env_conf = env.Clone()
    for n in list(externals.keys()):
      env_conf['need_'+n] = 0
    env_conf['need_'+name] = 1
    env_conf = link_flags(env_conf)
    libraries = [externals[n] for n in list(externals.keys()) if env_conf.get('need_'+n)]
...
def link_flags(env):
  ...
  for name,lib in list(externals.items()):
...
def objects(env,sources):
  ...
  libraries = [externals[name] for name in list(externals.keys()) if env['need_'+name]]
  for lib in libraries:
    if lib['callback'] is not None:
      env = env.Clone()
      lib['callback'](env)
...
def config_header(env,name,extra=()):
  ...
  for n,lib in list(externals.items()):
    if env.get('need_'+n):
...
def configure_blas(env,blas):
  ...
  for kind in kinds:
    ...
    if env['use_'+kind]:
      ...
      blas['requires'] = [kind]
      return 1
  print("disabling blas: can't find any variant, tried %s, and %s"%(', '.join(kinds[:-1]),kinds[-1]), file=sys.stderr)
  return 0
